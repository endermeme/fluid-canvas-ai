// Preset Game Participation Utils
...
export const addParticipant = async (gameId: string, playerName: string) => {
  try {
    ...
    console.log('Preset game participant added:', { gameId, playerName });
  } catch (error) {
...
export const updateParticipantActivity = async (gameId: string, playerName: string) => {
  try {
    const { error } = await supabase.rpc('update_preset_participant_activity', {
    ...
    console.error('Error updating preset participant activity:', error);
  }
};