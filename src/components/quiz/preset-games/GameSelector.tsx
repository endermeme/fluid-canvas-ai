    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-b from-primary/5 to-background">
      <div className="w-full max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent mb-3">
            Chọn Loại Trò Chơi
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 max-w-6xl mx-auto">
          {gameTypes.map((game) => (
            <Card 
              key={game.id}
              className={`p-6 cursor-pointer transform transition-all duration-300 ease-out will-change-transform ${
            ...
            >
              <div className="flex flex-col items-center text-center gap-4 h-full">
                <div className="p-3 bg-primary/10 rounded-full transition-transform duration-300 ease-out will-change-transform">
                  {game.icon}
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold mb-2 transition-colors duration-300">{game.name}</h3>
                  <p className="text-sm text-muted-foreground leading-relaxed transition-colors duration-300">
                    {game.description}
                  </p>
                </div>
                <div className="mt-auto w-full">
                  <div className={`flex items-center justify-center px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-300 ease-out ${
                    game.isBackButton 
                    ...
                    {game.isBackButton ? (
                      <>
                        <ArrowLeft className="h-3 w-3 mr-1 transition-transform duration-300" />
                        <span className="transition-all duration-300">Về trang chủ</span>
                      </>
                    ) : (
                      <>
                        <Sparkles className="h-3 w-3 mr-1 transition-transform duration-300" />
