...
const FlashcardsTemplate: React.FC<FlashcardsTemplateProps> = ({
  ...
  return <div className="flex flex-col h-screen max-h-screen overflow-hidden bg-gradient-to-b from-background to-background/80">
      <div className="flex-shrink-0 p-3 sm:p-4 pt-16 sm:pt-20 py-[8px]">
      ...
      <div className="flex-1 flex items-center justify-center p-3 sm:p-4 min-h-0 overflow-hidden">
        <div className="w-full max-w-5xl h-full max-h-[50vh] sm:max-h-[60vh] cursor-pointer relative group" onClick={handleFlip} style={{
        transformStyle: 'preserve-3d',
        transition: 'transform 0.6s',
        transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)'
      }}>
          <Card className="absolute inset-0 flex items-center justify-center p-4 sm:p-6 md:p-8 bg-gradient-to-br from-primary/5 to-background backdrop-blur-sm border-2 border-primary/20 shadow-lg group-hover:shadow-xl transition-all duration-300 overflow-auto" style={{
          backfaceVisibility: 'hidden'
        }}>
            <div className="text-center max-w-full h-full flex flex-col justify-center">
              <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2">
                Nhấn để lật thẻ
                {autoFlip && !isFlipped && <div className="mt-1 flex items-center justify-center">
          ...
          <Card className="absolute inset-0 flex items-center justify-center p-4 sm:p-6 md:p-8 bg-gradient-to-br from-primary/10 to-primary/5 backdrop-blur-sm border-2 border-primary/30 shadow-lg overflow-auto" style={{
          backfaceVisibility: 'hidden',
      ...
      <div className="flex-shrink-0 p-3 sm:p-4 bg-background/40 backdrop-blur-sm border-t border-primary/10">
        <div className="flex flex-col gap-2 sm:gap-3">
          <div className="grid grid-cols-4 gap-1 sm:gap-2">
            <Button variant="outline" onClick={handlePrevCard} disabled={currentCard === 0} className="bg-background/70 border-primary/20 text-xs sm:text-sm p-1 sm:p-2" size="sm">
              <ArrowLeft className="h-3 w-3 sm:h-4 sm:w-4 sm:mr-1" />
              <span className="hidden sm:inline">Trước</span>
            </Button>
            
            <Button variant="outline" onClick={handleFlip} className="col-span-2 bg-background/70 border-primary/20 text-xs sm:text-sm p-1 sm:p-2" size="sm">
              {isFlipped ? "Xem mặt trước" : "Lật thẻ"}
            </Button>
            
            <Button variant="outline" onClick={handleNextCard} disabled={currentCard === cards.length - 1} className="bg-background/70 border-primary/20 text-xs sm:text-sm p-1 sm:p-2" size="sm">
          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-between">
            <Button variant={autoFlip ? "default" : "outline"} size="sm" className={`flex-1 text-xs sm:text-sm ${autoFlip ? 'bg-primary/90' : 'bg-background/70 border-primary/20'}`} onClick={toggleAutoFlip}>
            ...
            <Button variant="outline" size="sm" onClick={handleRestart} className="bg-background/70 border-primary/20 flex-1 text-xs sm:text-sm">
          ...
          <ToggleGroup type="single" variant="outline" className="grid grid-cols-2 gap-1 sm:gap-2">
            <ToggleGroupItem value="unknown" onClick={() => handleMarkCard('unknown')} className="border border-red-300 text-red-600 data-[state=on]:bg-red-100 data-[state=on]:text-red-700 text-xs sm:text-sm p-2">
              <X className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" />
              <span className="hidden sm:inline">Chưa thuộc</span>
              <span className="sm:hidden">Chưa thuộc</span>
            </ToggleGroupItem>
            
            <ToggleGroupItem value="known" onClick={() => handleMarkCard('known')} className="border border-green-300 text-green-600 data-[state=on]:bg-green-100 data-[state=on]:text-green-700 text-xs sm:text-sm p-2">
