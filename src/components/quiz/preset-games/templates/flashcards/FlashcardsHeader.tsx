...
const FlashcardsHeader: React.FC<FlashcardsHeaderProps> = ({
  ...
  return (
    <div className="flex-shrink-0 p-2 sm:p-3 bg-background/95 backdrop-blur-sm border-b border-primary/10">
      <div className="flex justify-between items-center mb-2">
        <div className="text-sm font-medium px-3 py-1 bg-primary/10 rounded-full">
          {currentCard + 1}/{totalCards}
        </div>
        <div className="text-sm font-medium flex gap-2">
          <span className="px-2 py-1 bg-green-100/30 text-green-600 rounded-full text-xs">
            <span className="hidden sm:inline">Thuộc: </span>{stats.known}
          </span>
          <span className="px-2 py-1 bg-red-100/30 text-red-600 rounded-full text-xs">
