...
const QuizTemplate: React.FC<QuizTemplateProps> = ({
  ...
  return <div className="flex flex-col h-screen max-h-screen overflow-hidden bg-gradient-to-br from-blue-50/80 via-sky-50/80 to-blue-100/80 dark:from-blue-950/80 dark:via-sky-950/80 dark:to-blue-950/80">
      ...
      {/* Nội dung chính */}
      <div className="flex-1 min-h-0 overflow-auto p-6 sm:p-8 py-0">
            ...
            <Card className="p-8 sm:p-10 bg-gradient-to-br from-white/90 to-white/80 backdrop-blur-sm border-primary/20 shadow-xl">
              ...
              <div className="space-y-5">
                <AnimatePresence>
                  ...
                  return <motion.button key={index} initial={{
                    opacity: 0,
                    x: -30
                  }} animate={{
                    opacity: 1,
                    ...
                    scale: 0.98
                  } : {}} onClick={() => handleOptionSelect(index)} disabled={isAnswered} className={`
                          w-full p-6 sm:p-8 text-left rounded-2xl transition-all duration-300 border-2 
                          ${showCorrect ? 'bg-gradient-to-r from-green-100/90 to-green-50/90 border-green-400 shadow-xl shadow-green-200/50 dark:from-green-900/40 dark:to-green-800/30 dark:border-green-500' : showIncorrect ? 'bg-gradient-to-r from-red-100/90 to-red-50/90 border-red-400 shadow-xl shadow-red-200/50 dark:from-red-900/40 dark:to-red-800/30 dark:border-red-500' : isAnswered && isCorrect ? 'bg-gradient-to-r from-green-100/90 to-green-50/90 border-green-400 shadow-xl shadow-green-200/50 dark:from-green-900/40 dark:to-green-800/30 dark:border-green-500' : 'bg-gradient-to-r from-white/90 to-gray-50/90 border-gray-200 hover:border-primary/50 hover:shadow-xl hover:shadow-primary/20 dark:from-slate-800/90 dark:to-slate-700/90 dark:border-slate-600'}
                          ${!isAnswered ? 'hover:bg-gradient-to-r hover:from-primary/10 hover:to-primary/15 cursor-pointer' : 'cursor-default'}
                        `}>
                        <div className="flex items-center">
                          <div className="flex-shrink-0 mr-6">
                            {showCorrect ? <motion.div initial={{
                          scale: 0
                        }} animate={{
                          scale: 1
                        }} transition={{
                          ...
                          damping: 15
                              ...
                              </motion.div> : showIncorrect ? <motion.div initial={{
                          scale: 0
                        }} animate={{
                          scale: 1
                        }} transition={{
                          ...
                          damping: 15
                                ...
                                ${isAnswered && isCorrect ? 'bg-green-500 border-green-500 text-white' : 'bg-primary/10 border-primary/30 text-primary/70'}
                              `}>
                          ...
                          <span className={`
                            ...
                            ${showCorrect ? 'text-green-800 dark:text-green-200 font-semibold' : showIncorrect ? 'text-red-800 dark:text-red-200 font-medium' : 'text-foreground'}
                          `}>
                            {option}
                          </span>
                        </div>
      ...
      opacity: 1
        ...
        <div className="max-w-4xl mx-auto flex gap-4">
          <Button variant="outline" size="lg" onClick={handleRestart} className="bg-white/70 backdrop-blur-sm border-primary/20 hover:bg-white/90 flex-1 text-lg py-6">
            <RefreshCw className="h-5 w-5 mr-3" />
            Làm lại
          </Button>
          
          <Button onClick={handleNextQuestion} disabled={!isAnswered} size="lg" className={`
              flex-2 min-w-0 text-lg py-6
              ${isAnswered ? 'bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary shadow-xl' : 'bg-primary/50 cursor-not-allowed'}
            `}>
